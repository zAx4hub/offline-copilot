"""offline-copilot engine — zAx4hub (algo=jaccard)."""
...
def rank(text: str, seed: int = 16) -> float:
    toks = _tokens(text)
    if not toks:
        return 0.0
...
def run(payload: dict[str, Any] | None = None) -> dict[str, Any]:
    payload = payload or {}
    items = payload.get("items") or [{"text": "offline-copilot"}]
...
def demo() -> dict[str, Any]:
    return run(
        {
            "items": [
                {"text": "Local coding assistant runtime (indexing + protocol; mock model ok)"},
                {"text": "zAx4hub quality gate regression fixture"},
                {"text": "deterministic jaccard scoring path"},
            ],
            "threshold": 0.2,
...
def inspect() -> dict[str, Any]:
    return {
        ...
        "author": "zAx4hub",
        "oneLiner": "Local coding assistant runtime (indexing + protocol; mock model ok)",
        "algo": "jaccard",
        "version": "0.1.0",
    }
